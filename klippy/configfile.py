...
import ConfigParser, re, os, StringIO, logging
...
class ConfigWrapper:
    ...
    def getfloat(self, option, default=sentinel, minval=None, maxval=None, above=None, below=None):
    ...
    def get_prefix_extra_sections(self, prefixes):
        return [self.getsection(s) for s in self.fileconfig.sections()
                if not s.startswith(tuple(["printer",]+prefixes))]
...
class PrinterConfig:
    ...
    def read_main_config(self):
        ...
        return cfg
    def check_unused_options(self, config, hal):
        fileconfig = config.fileconfig
